...
CREATE TABLE SALES (
    ...
    payment_method TEXT NOT NULL
        CHECK (payment_method IN ('cash', 'card', 'mobile_pay')),
...
CREATE TABLE INVENTORY (
    ...
    safety_stock INTEGER CHECK(safety_stock >= 0),
    last_updated TEXT NOT NULL,
    store_price  REAL CHECK(store_price >= 0),
...
CREATE TABLE ORDERDETAIL (
    ...
    supply_price   REAL CHECK(supply_price >= 0),
...
-- DELIVERY: 동일 (order_id, product_id)에 대해 여러 번 입고 가능
-- → surrogate key delivery_id 사용, OrderDetail:Delivery = 1:N
...
CREATE INDEX idx_productcategorymap_product
    ON PRODUCTCATEGORYMAP(product_id);

CREATE INDEX idx_productcategorymap_category
    ON PRODUCTCATEGORYMAP(category_id);
...
CREATE INDEX idx_supplier_brand_supplier
    ON SUPPLIER_BRAND(supplier_id);

CREATE INDEX idx_supplier_brand_brand
    ON SUPPLIER_BRAND(brand_id);
